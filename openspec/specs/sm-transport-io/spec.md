...
State machine transport I/O for TCP protocols. When a state machine holds a non-null `IConnectionTransport`, it owns the read loop (inbound) and flush cycle (outbound) directly, bypassing the legacy port-based data path. Covers `RequestRead`/`OnReadCompleted`, `RequestFlush`/`OnFlushCompleted`, generation guards for stale async results, the `OnAsyncResult` dispatch method, `IClientStageOperations`/`IServerStageOperations` `Self` exposure, and the `IBufferWriter<byte>` encoder contract.
...
- **AND** inbound data keeps arriving through `DecodeServerData`/`DecodeClientData` as before
