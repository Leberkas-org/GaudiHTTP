...
- [ ] 2.7 Migrate H1.1 Client SM tests — BLOCKED: H1.1 Client SM does not extend TcpStateMachineBase, needs pipe-transport migration first (`Http11StateMachineSpec`, `Http11ClientEncoderSpec`, etc.) to `InMemoryTransport`.
...
- [ ] 4.2 `Http11ClientStateMachine.DecodeServerData`: remove TransportData fallback. Add throw.
...
- [ ] 4.5 Migrate remaining H1 tests that feed data via `DecodeClientData(TransportData.Rent(...))` to use `InMemoryTransport.Feed()` + `TryHandleAsyncResult(ReadCompleted(...))`.
...
- [ ] 5.7 Migrate H2 Client SM tests to `InMemoryTransport`.
- [ ] 5.8 Migrate H2 Server SM tests to `InMemoryTransport`.
...
- [ ] 6.3 Migrate remaining H2 tests that feed data via `DecodeClientData`/`DecodeServerData` with `TransportData` to `InMemoryTransport`.

## 7. Body Pumps

- [ ] 7.1 `SerialBodyPump` / `PumpSlot`: remove WireBuffer.Rent/Wrap paths where transport is available. Verify all EmitDataFrames call sites go through pipe.
- [ ] 7.2 `BufferedBodyReader`: remove any WireBuffer.Rent paths that have transport alternatives.
- [ ] 7.3 Migrate body pump tests that assert on WireBuffer-wrapped outbound data.

## 8. Cleanup

- [ ] 8.1 Delete `WireBufferWriter` (`src/GaudiHTTP/Protocol/WireBufferWriter.cs`).
- [ ] 8.2 Delete `WireBufferTestExtensions` from `GaudiHTTP.Tests/TestSupport/`.
- [ ] 8.3 Remove WireBuffer imports from all modified TCP protocol files (verify no remaining usage via grep).
- [ ] 8.4 Run full test suite (`dotnet run --project GaudiHTTP.Tests/GaudiHTTP.Tests.csproj`) and verify all pass.
- [ ] 8.5 Run stage tests and integration tests to verify no regressions.
