...
### Requirement: Pre-connect client SM buffers via OnOutbound
**Reason**: The pre-connect WireBuffer→TransportData→OnOutbound buffering path is eliminated.
Transport is always connected before the SM produces outbound data. The `EmitWireBuffer` helper
and its `else` branch are deleted from all TCP SMs.
**Migration**: No production code replacement needed — the path was dead code. Test code migrates
to `InMemoryTransport` assertions on `WrittenBytes`.
