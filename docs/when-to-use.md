# When to Use GaudiHTTP

GaudiHTTP is not a drop-in "faster HttpClient/Kestrel". It is an HTTP stack built on Akka.Streams
...
| HTTP/2 server endpoints (plaintext, JSON) | **GaudiServer** (1.4–1.5× Kestrel) |
| Concurrent downloads over HTTP/2 or HTTP/3 | **GaudiHTTP client** (2–3.5× HttpClient) |
| HTTP/1.1 pipelined requests on a single connection | **GaudiHTTP client** (up to 4.7× HttpClient) |
| Streaming, SSE, backpressure end-to-end | **GaudiHTTP (both sides)** |
| Actor-based backends (Akka.NET) | **GaudiServer** — shares your `ActorSystem` |
| Bulk request pipelines (fire thousands, drain results) | **GaudiHTTP client channel API** |
| HTTP/3 (QUIC) at any scale | HttpClient / Kestrel (GaudiHTTP H3 is 2–7× slower) |
...
- **HTTP/1.1 pipelining on a single connection.** At 256 concurrent requests over one connection,
  GaudiHTTP delivers **4.7× the throughput** (73K vs 15K req/s) of HttpClient. At 64 concurrent
...
- **HTTP/2 and HTTP/3 concurrent downloads.** Downloading 1 MB payloads across 32 connections,
  GaudiHTTP is **2.3× faster on H2** (2,727 vs 1,199 req/s) and **3.5× faster on H3** (613 vs
...
- **HTTP/1.1 concurrent light requests at moderate scale.** At 512 concurrent light GETs,
  GaudiHTTP is **1.6× faster** (66K vs 42K req/s) than HttpClient.
- **HTTP/2 single-connection multiplexing.** At 64 concurrent requests on one H2 connection,
  GaudiHTTP delivers **1.5× the throughput** (49K vs 33K req/s).
...
- **Very high concurrency (4096+).** GaudiHTTP's SendAsync API currently crashes at 4096
...
- **HTTP/2 plaintext and JSON at high concurrency.** At 256 concurrent requests, GaudiServer
...
- **Streaming responses with real backpressure.** Return an Akka Streams `Source` (SSE, long
  ...
  growing a buffer.
- **Actor integration.** GaudiServer reuses your `ActorSystem` from DI; HTTP connections and
  domain actors share supervision, dispatchers, and tracing (see [Scenarios](/scenarios)).

### Where Kestrel is the better tool

- **HTTP/3 (QUIC) — significantly.** GaudiServer is **1.4–3.9× slower** than Kestrel across
...
- **Larger response bodies on HTTP/2.** The Fortunes benchmark (larger HTML responses) shows a
  ...
  plaintext/JSON where GaudiServer leads. This points to a body-write or serialization
  bottleneck specific to larger response payloads.
- **Per-request allocations.** GaudiServer allocates roughly **2.5–3× more** per request than
...
Running GaudiHTTP on both ends pays off when the *pipeline* is the product:

- **HTTP/2 service-to-service.** GaudiHTTP client's H2 download advantage (2.3×) combined with
  GaudiServer's H2 serving advantage (1.4–1.5×) makes a compelling end-to-end H2 story.
...
Numbers above come from the repo's benchmark suite (`GaudiHTTP.Benchmarks`): Ryzen 7 5800X
...
only. Re-run with `dotnet run -c Release --project GaudiHTTP.Benchmarks` to reproduce on your
hardware.
