...
namespace TurboHTTP.StreamTests.Acceptance.H11;

public sealed class HandlerPipelineSpec : EngineTestBase
{
    ...
    public async Task HandlerPipeline_should_work_with_compression_pipeline()
    {
        ...
        var map = new ResponseMap()
            .On("/compress/gzip/1", _ => new HttpResponseMessage(HttpStatusCode.OK)
            {
        ...
        var fake = ResponseMapFake.Create(map);
        var decompression = BidiFlow.FromGraph(new ContentEncodingBidiStage(automaticDecompression: true));
        var handlerStage = BidiFlow.FromGraph(new HandlerBidiStage(handler, 0));
        var flow = handlerStage.Atop(decompression).Atop(fake)
            .Join(Flow.FromFunction<HttpRequestMessage, HttpResponseMessage>(_ => new HttpResponseMessage()));

        var tcs = new TaskCompletionSource<HttpResponseMessage>();
        _ = Source.Single(new HttpRequestMessage(HttpMethod.Get, "http://localhost/compress/gzip/1"))
            .Via(flow)
            .RunWith(Sink.ForEach<HttpResponseMessage>(res => tcs.TrySetResult(res)), Materializer);
    ...
    public async Task HandlerPipeline_should_work_with_cookie_pipeline()
    {
        ...
        var map = new ResponseMap()
            ...
            .On("/interaction/echo-all-headers", EchoCookies);
        ...
        var seedResponse = await SendWithHandlerCookieAsync(map,
