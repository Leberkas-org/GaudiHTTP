...
public sealed class RetrySpec : EngineTestBase
{
    ...
    [Fact(Timeout = 5000)]
    [Trait("RFC", "RFC9110-15.5.30")]
    public async Task Get_should_retry_429_and_eventually_return_429()
    {
        var map = new ResponseMap()
            .On("/retry/429", _ => new HttpResponseMessage(HttpStatusCode.TooManyRequests));

        var response = await SendAsync(map,
            new HttpRequestMessage(HttpMethod.Get, "http://localhost/retry/429"));
