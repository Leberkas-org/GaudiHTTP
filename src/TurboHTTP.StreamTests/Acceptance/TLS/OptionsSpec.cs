...
public sealed class OptionsSpec : EngineTestBase
{
    ...
    public async Task PreAuthenticate_should_inject_authorization_header_over_https()
    {
        ...
        var response = await SendAsync(map,
            new HttpRequestMessage(HttpMethod.Get, "https://localhost/auth"), options);
    ...
    [Fact(Timeout = 5000)]
    [Trait("RFC", "RFC9110-11.6.1")]
    public async Task PreAuthenticate_should_send_basic_auth_header_with_correct_credentials_over_https()
    {
        var map = CreateAuthMap();
        var options = CreateOptions(
        ...
        var response = await SendAsync(map,
            new HttpRequestMessage(HttpMethod.Get, "https://localhost/auth/echo"), options);
    ...
    [Fact(Timeout = 5000)]
    [Trait("RFC", "RFC9110-11.6.1")]
    public async Task Auth_should_return_401_when_no_credentials_configured_over_https()
    {
        var map = CreateAuthMap();
        var options = CreateOptions();
