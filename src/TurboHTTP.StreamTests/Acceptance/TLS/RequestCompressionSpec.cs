...
public sealed class RequestCompressionSpec : EngineTestBase
{
    ...
    public async Task RequestCompression_should_send_and_verify_gzip_request_body_over_https()
    {
        var payload = MakePayload(4 * 1024);
        var request = new HttpRequestMessage(HttpMethod.Post, "https://localhost/compress/verify-gzip")
        {
    ...
    [Fact(Timeout = 5000)]
    [Trait("RFC", "RFC9110-8.4")]
    public async Task RequestCompression_should_send_and_verify_deflate_request_body_over_https()
    {
        var payload = MakePayload(4 * 1024);
        var request = new HttpRequestMessage(HttpMethod.Post, "https://localhost/compress/verify-deflate")
        {
            Version = HttpVersion.Version11,
            Content = new ByteArrayContent(payload)
        };
    ...
    [Fact(Timeout = 5000)]
    [Trait("RFC", "RFC9110-8.4")]
    public async Task RequestCompression_should_send_and_verify_brotli_request_body_over_https()
    {
        var payload = MakePayload(4 * 1024);
        var request = new HttpRequestMessage(HttpMethod.Post, "https://localhost/compress/verify-br")
        {
            Version = HttpVersion.Version11,
            Content = new ByteArrayContent(payload)
        };
    ...
    [Fact(Timeout = 5000)]
    [Trait("RFC", "RFC9110-8.4")]
    public async Task RequestCompression_should_not_compress_small_body_below_threshold_over_https()
    {
        var payload = MakePayload(100);
        var request = new HttpRequestMessage(HttpMethod.Post, "https://localhost/compress/echo")
        {
            Version = HttpVersion.Version11,
            Content = new ByteArrayContent(payload)
        };
    ...
    [Fact(Timeout = 5000)]
    [Trait("RFC", "RFC9110-8.4")]
    public async Task RequestCompression_should_set_content_encoding_header_correctly_for_gzip_over_https()
    {
        var payload = MakePayload(4 * 1024);
        var request = new HttpRequestMessage(HttpMethod.Post, "https://localhost/compress/echo")
        {
    ...
    [Fact(Timeout = 5000)]
    [Trait("RFC", "RFC9110-8.4")]
    public async Task RequestCompression_should_roundtrip_compressed_request_and_decompressed_response_over_https()
    {
        var payload = MakePayload(4 * 1024);

        var postRequest = new HttpRequestMessage(HttpMethod.Post, "https://localhost/compress/verify-gzip")
        {
            Version = HttpVersion.Version11,
            Content = new ByteArrayContent(payload)
        };
        ...
        var getRequest = new HttpRequestMessage(HttpMethod.Get, "https://localhost/compress/gzip/1")
        {
            Version = HttpVersion.Version11
        };
