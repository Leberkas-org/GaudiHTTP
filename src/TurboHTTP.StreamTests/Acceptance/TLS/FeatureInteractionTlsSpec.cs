...
public sealed class FeatureInteractionTlsSpec : EngineTestBase
{
    ...
    public async Task Redirect_should_preserve_cookies_across_hops()
    {
        ...
        var map = new ResponseMap()
            .On("/cookie/set-and-redirect", _ =>
            {
                ...
                r.Headers.Location = new Uri("https://localhost/cookie/echo");
                return r;
            })
    ...
    public async Task Compressed_response_should_be_served_from_cache()
    {
        ...
        var res1 = await SendCacheAsync(map,
            new HttpRequestMessage(HttpMethod.Get, "https://localhost/interaction/cache-gzip"), store);
    ...
    public async Task Retry_should_follow_after_redirect_target_returns_503()
    {
        ...
        var map = new ResponseMap()
            ...
            {
                var r = new HttpResponseMessage(HttpStatusCode.Found);
                r.Headers.Location = new Uri("https://localhost/retry/succeed-after/2");
                return r;
            })
        ...
        var response = await SendRedirectRetryAsync(map,
            new HttpRequestMessage(HttpMethod.Get, "https://localhost/interaction/redirect-succeed-after/2"),
    ...
    public async Task Cookie_should_survive_retry_cycle()
    {
        ...
        await SendCookieRetryAsync(map,
            new HttpRequestMessage(HttpMethod.Get, "https://localhost/cookie/set/auth-token/abc123"),
            jar, new RetryPolicy { MaxRetries = 3 });

        var retryRes = await SendCookieRetryAsync(map,
            new HttpRequestMessage(HttpMethod.Get, "https://localhost/retry/succeed-after/2"),
            jar, new RetryPolicy { MaxRetries = 3 });
        Assert.Equal(HttpStatusCode.OK, retryRes.StatusCode);

        var echoRes = await SendCookieRetryAsync(map,
            new HttpRequestMessage(HttpMethod.Get, "https://localhost/cookie/echo"),
            jar, new RetryPolicy { MaxRetries = 3 });
    ...
    public async Task Vary_header_should_separate_cache_entries_with_cookies_active()
    {
        ...
        var req1 = new HttpRequestMessage(HttpMethod.Get, "https://localhost/cache/vary/Accept-Language");
        req1.Headers.Add("Accept-Language", "en");

        var req2 = new HttpRequestMessage(HttpMethod.Get, "https://localhost/cache/vary/Accept-Language");
        req2.Headers.Add("Accept-Language", "de");

        var req3 = new HttpRequestMessage(HttpMethod.Get, "https://localhost/cache/vary/Accept-Language");
        ...
        var map = new ResponseMap()
            .On(req => req.RequestUri?.AbsolutePath.StartsWith("/interaction/cookie-hop/") == true, req =>
            {
                ...
                r.Headers.Location = hop < 3
                    ? new Uri($"https://localhost/interaction/cookie-hop/{hop + 1}")
                    : new Uri("https://localhost/cookie/echo");
                return r;
            })
            .On("/cookie/echo", EchoCookies);

        var response = await SendCookieRedirectAsync(map,
            new HttpRequestMessage(HttpMethod.Get, "https://localhost/interaction/cookie-hop/1"), jar);
    ...
    public async Task Cache_hit_should_bypass_retry_logic()
    {
        ...
        var res1 = await SendCacheRetryAsync(map,
            new HttpRequestMessage(HttpMethod.Get, "https://localhost/cache/max-age/3600"),
