...
public sealed class TurboServerFixture : IAsyncLifetime
{
    ...
    private static void RegisterEndpoints(WebApplication app)
    {
        ...
        app.MapWhen(ctx => ctx.Request.Path.StartsWithSegments("/api"), api =>
        {
            ...
            api.UseRouting();
            api.UseEndpoints(endpoints => { endpoints.MapGet("/api/data", () => Results.Ok(new { value = 42 })); });
        });
...
#pragma warning restore CS0162
        ...
        app.MapGet("/items/{category}/{id:int}", (string category, int id) => Results.Ok(new { category, id }));
        ...
        {
            var body = "exact-length-body"u8.ToArray();
