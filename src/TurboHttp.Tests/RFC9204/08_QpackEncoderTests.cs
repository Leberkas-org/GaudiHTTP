...
public sealed class QpackEncoderTests
{
    ...
    [Fact(DisplayName = "RFC9204-4.5.2-ENC-001: Static table exact match encodes as indexed")]
    ...
    [Fact(DisplayName = "RFC9204-4.5.2-ENC-002: Multiple static table headers all indexed")]
    ...
    // ── Dynamic table insert + reference (§4.5.2) ──────────────────────

    /// RFC 9204 §4.5.2 — New header inserted into dynamic table and referenced
    [Fact(DisplayName = "RFC9204-4.5.2-ENC-003: Dynamic table insert and reference")]
    ...
    [Fact(DisplayName = "RFC9204-4.5.2-ENC-004: Dynamic table reuse on second encode")]
    public void Should_ReuseDynamicEntry_When_AlreadyInserted()
    {
        ...
        var output = new ArrayBufferWriter<byte>();
        encoder.Encode(headers2, output);
    ...
    // ── Literal headers without indexing (§4.5.6) ──────────────────────

    /// RFC 9204 §4.5.6 — Literal without name reference when table is disabled
    [Fact(DisplayName = "RFC9204-4.5.6-ENC-005: Literal without name reference")]
    ...
    // ── NEVERINDEX for sensitive headers (§4.5.4, §4.5.6) ──────────────

    /// RFC 9204 §7.1 — Authorization header uses never-indexed representation
    [Fact(DisplayName = "RFC9204-7.1-ENC-006: Authorization header is never-indexed")]
    ...
    [Fact(DisplayName = "RFC9204-7.1-ENC-007: Cookie header is never-indexed")]
    ...
    // ── Required Insert Count prefix (§4.5.1) ──────────────────────────

    /// RFC 9204 §4.5.1.1 — Required Insert Count encoding
    [Fact(DisplayName = "RFC9204-4.5.1-ENC-008: Required Insert Count prefix encoded correctly")]
    ...
    // ── Huffman encoding support (§4.1.2) ──────────────────────────────

    /// RFC 9204 §4.1.2 — Huffman encoding is applied when shorter
    [Fact(DisplayName = "RFC9204-4.1.2-ENC-009: Huffman encoding applied for string literals")]
    public void Should_UseHuffman_When_Shorter()
    {
        ...
        Assert.True(output.WrittenCount > 2); // prefix + at least one header
    }

    // ── Encoder instructions emitted as side-effect ────────────────────

    /// RFC 9204 §4.3 — Encoder instructions emitted when inserting with static name ref
    [Fact(DisplayName = "RFC9204-4.3-ENC-010: Encoder instructions emitted on dynamic insert")]
    ...
    [Fact(DisplayName = "RFC9204-4.5.4-ENC-011: Literal with static name ref when table full")]
    ...
    [Fact(DisplayName = "RFC9204-ENC-012: Empty header name throws")]
    public void Should_Throw_When_EmptyHeaderName()
    {
        ...
        var output = new ArrayBufferWriter<byte>();
        Assert.Throws<QpackException>(() => encoder.Encode(headers, output));
    }
}
