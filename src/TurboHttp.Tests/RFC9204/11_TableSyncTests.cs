...
/// </summary>
public sealed class QpackTableSyncTests
{
    // ── Encoder-Decoder sync (§2.1.1) ────────────────────────────────────

    /// RFC 9204 §2.1.1 — Encoder instructions synchronize decoder dynamic table
    [Fact(DisplayName = "RFC9204-2.1.1-SYNC-001: Encoder instructions sync decoder table")]
    ...
    [Fact(DisplayName = "RFC9204-2.1.1-SYNC-002: Insert count tracks across multiple header blocks")]
    ...
    [Fact(DisplayName = "RFC9204-2.1.1-SYNC-003: Reused headers skip inserts on second encode")]
    ...
    // ── Blocked streams (§2.1.2) ─────────────────────────────────────────

    /// RFC 9204 §2.1.2 — Stream blocks when required insert count exceeds known
    [Fact(DisplayName = "RFC9204-2.1.2-SYNC-004: Stream blocks when required insert count exceeds known")]
    ...
    [Fact(DisplayName = "RFC9204-2.1.2-SYNC-005: Blocked stream resolved when insert count reached")]
    ...
    [Fact(DisplayName = "RFC9204-2.1.2-SYNC-006: Multiple blocked streams resolved in batch")]
    ...
    // ── Known Received Count (§4.4) ──────────────────────────────────────

    /// RFC 9204 §4.4.3 — Insert Count Increment updates Known Received Count
    [Fact(DisplayName = "RFC9204-4.4.3-SYNC-007: Insert Count Increment updates KRC")]
    public void Should_UpdateKnownReceivedCount_ViaInsertCountIncrement()
    {
        ...
        var increment = sync.WriteInsertCountIncrement(buf);
        ...
        encoderSync.ProcessDecoderInstructions(buf.WrittenSpan);

        Assert.Equal(2, encoderSync.KnownReceivedCount);
    }

    /// RFC 9204 §4.4.2 — Stream Cancellation removes blocked stream
    [Fact(DisplayName = "RFC9204-4.4.2-SYNC-008: Stream Cancellation removes blocked stream")]
    public void Should_RemoveBlockedStream_OnStreamCancellation()
    {
        ...
        var buf = new ArrayBufferWriter<byte>(16);
        QpackDecoderInstructionWriter.WriteStreamCancellation(12, buf);
        sync.ProcessDecoderInstructions(buf.WrittenSpan);
