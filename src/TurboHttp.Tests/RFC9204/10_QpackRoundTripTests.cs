...
{
    // ── Static-only roundtrips (§4.5.2) ─────────────────────────────────

    /// RFC 9204 §4.5.2 — Static-only headers roundtrip through encode→decode
    [Fact(DisplayName = "RFC9204-4.5-RT-001: Static-only headers roundtrip")]
    ...
    [Fact(DisplayName = "RFC9204-4.5-RT-002: Static indexed + literal with static name roundtrip")]
    ...
    // ── Dynamic table roundtrips (§4.5.2 / §4.5.4) ─────────────────────

    /// RFC 9204 §4.5.2 — Dynamic table entries roundtrip with synchronised tables
    [Fact(DisplayName = "RFC9204-4.5-RT-003: Dynamic table entries roundtrip")]
    ...
    [Fact(DisplayName = "RFC9204-4.5-RT-004: Repeated headers reuse dynamic table")]
    ...
    // ── Sensitive headers / NEVERINDEX roundtrip (§7.1) ─────────────────

    /// RFC 9204 §7.1 — Sensitive headers (Authorization, Cookie) roundtrip as NEVERINDEX
    [Fact(DisplayName = "RFC9204-7.1-RT-005: Sensitive headers NEVERINDEX roundtrip")]
    ...
    [Fact(DisplayName = "RFC9204-7.1-RT-006: Mixed sensitive + normal headers roundtrip")]
    ...
    // ── Full header list roundtrips ─────────────────────────────────────

    /// RFC 9204 §4.5 — Large header list with all representation types roundtrips
    [Fact(DisplayName = "RFC9204-4.5-RT-007: Large header list preserves all pairs")]
    ...
    [Fact(DisplayName = "RFC9204-4.5-RT-008: Empty header list roundtrips")]
