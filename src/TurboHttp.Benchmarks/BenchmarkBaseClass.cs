...
public abstract class BenchmarkBaseClass
{
    ...
    /// <summary>Port on which the Kestrel test server is listening. Set in GlobalSetup.</summary>
    protected int KestrelPort { get; set; }
    ...
    /// Uses the loopback address 127.0.0.1 and the port discovered during GlobalSetup.
    /// </summary>
    ...
    public virtual void GlobalSetup()
    {
        lock (_serverLock)
        {
            ...
            _serverRefCount++;
            KestrelPort = _sharedServer.Port;
        }
    }
