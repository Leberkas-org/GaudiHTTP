...
[Collection("TLS")]
[Obsolete("Replaced by StreamTests.Acceptance.TLS.ResilienceSpec")]
public sealed class ResilienceSpec
{
    ...
    [Fact(Timeout = 20000)]
    public async Task Content_length_mismatch_should_cause_exception_or_timeout()
    {
    ...
    [Fact(Timeout = 20000)]
    public async Task Corrupt_gzip_should_cause_graceful_failure()
    {
    ...
    [Fact(Timeout = 20000)]
    public async Task Corrupt_brotli_should_cause_graceful_failure()
    {
        // ContentEncodingBidiStage catches decompression failures and passes the raw
        // response through unmodified — no exception, no hang, no crash.
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(5));
    ...
    [Fact(Timeout = 20000)]
    public async Task Truncated_body_should_be_detected()
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(5));
        await using var helper = CreateClient();
    ...
    [Fact(Timeout = 20000)]
    public async Task SlowBody_within_timeout_should_succeed()
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(30));
        await using var helper = CreateClient();
