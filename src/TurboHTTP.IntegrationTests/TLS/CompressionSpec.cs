...
[Collection("TLS")]
[Obsolete("Replaced by StreamTests.Acceptance.TLS.CompressionSpec")]
public sealed class CompressionSpec
{
    ...
    [Fact(Timeout = 20000)]
    public async Task Gzip_response_should_be_transparently_decompressed_over_https()
    {
    ...
    [Fact(Timeout = 20000)]
    public async Task Deflate_response_should_be_transparently_decompressed_over_https()
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
        await using var helper = CreateClient();
    ...
    [Fact(Timeout = 20000)]
    public async Task Brotli_response_should_be_transparently_decompressed_over_https()
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
        await using var helper = CreateClient();
    ...
    [Fact(Timeout = 20000)]
    public async Task Identity_encoding_should_pass_through_unchanged_over_https()
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
        await using var helper = CreateClient();
    ...
    [Fact(Timeout = 20000)]
    public async Task Negotiate_accept_encoding_gzip_should_return_gzip_over_https()
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
        await using var helper = CreateClient();
    ...
    [Fact(Timeout = 20000)]
    public async Task Negotiate_accept_encoding_br_should_return_brotli_over_https()
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
        await using var helper = CreateClient();
    ...
    [Fact(Timeout = 20000)]
    public async Task Negotiate_no_accept_encoding_should_return_identity_over_https()
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
        await using var helper = CreateClient();
