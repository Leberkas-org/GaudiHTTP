...
[Collection("TLS")]
[Obsolete("Replaced by StreamTests.Acceptance.TLS.CacheSpec")]
public sealed class CacheSpec
{
    ...
    [Fact(Timeout = 20000)]
    public async Task Cache_should_serve_max_age_response_from_cache_on_second_request_over_https()
    {
    ...
    [Fact(Timeout = 20000)]
    public async Task Cache_should_send_if_none_match_for_etag_revalidation_over_https()
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
        await using var helper = CreateCacheClient();
    ...
    [Fact(Timeout = 20000)]
    public async Task Cache_should_send_if_modified_since_for_last_modified_revalidation_over_https()
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
        await using var helper = CreateCacheClient();
    ...
    [Fact(Timeout = 20000)]
    public async Task Cache_should_produce_different_entries_with_vary_header_over_https()
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
        await using var helper = CreateCacheClient();
    ...
    [Fact(Timeout = 20000)]
    public async Task Cache_should_respect_s_maxage_with_shared_cache_over_https()
    {
        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
