...
[Collection("H2")]
[Obsolete("Replaced by StreamTests.Acceptance.H2.ConcurrencySpec")]
public sealed class ConcurrencySpec : IAsyncLifetime
{
    ...
    {
        // Verifies HTTP/2 flow control WINDOW_UPDATE threshold is low enough to prevent
        // server send-window exhaustion when echoing large bodies back.
        var payload = new byte[10 * 1024];
        await using var ownHelper = ClientHelper.CreateClient(_server.H2Port, new Version(2, 0));

        using var cts = new CancellationTokenSource(TimeSpan.FromSeconds(50));

        for (var batch = 0; batch < 8; batch++)
        {
            var tasks = new Task<HttpResponseMessage>[64];
            for (var i = 0; i < 64; i++)
            {
