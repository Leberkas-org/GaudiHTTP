...
[Obsolete("Replaced by StreamTests.Acceptance.H3.ConnectionSpec")]
public sealed class ConnectionSpec : IAsyncLifetime
{
    ...
    public async Task Sequential_requests_should_reuse_same_connection()
    {
        ...
        var response1 = await _helper!.Client.SendAsync(
        ...
        var response2 = await _helper.Client.SendAsync(
            new HttpRequestMessage(HttpMethod.Get, "/hello"), cts.Token);
    ...
    public async Task Multiple_endpoints_should_be_served_on_same_connection()
    {
        ...
        var helloResponse = await _helper!.Client.SendAsync(
            new HttpRequestMessage(HttpMethod.Get, "/hello"), cts.Token);
        ...
        var headersResponse = await _helper.Client.SendAsync(
            new HttpRequestMessage(HttpMethod.Get, "/h3/many-headers"), cts.Token);
        ...
        var largeResponse = await _helper.Client.SendAsync(
    ...
    public async Task Post_with_body_followed_by_get_should_work_on_same_connection()
    {
        ...
        var postRequest = new HttpRequestMessage(HttpMethod.Post, "/h3/echo-binary")
        {
        ...
        var postResponse = await _helper!.Client.SendAsync(postRequest, cts.Token);
        ...
        var getResponse = await _helper.Client.SendAsync(
            new HttpRequestMessage(HttpMethod.Get, "/h3/settings"), cts.Token);
