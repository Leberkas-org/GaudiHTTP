...
public sealed class GaudiListenOptionsSpec
{
    [Fact(Timeout = 5000)]
    public void GaudiListenOptions_should_store_address_and_port()
    {
        var options = new GaudiListenOptions(IPAddress.Loopback, 5001);
        Assert.Equal(IPAddress.Loopback, options.Address);
        Assert.Equal((ushort)5001, options.Port);
    }

    [Fact(Timeout = 5000)]
    public void GaudiListenOptions_should_default_protocols_to_Http1AndHttp2()
    {
        var options = new GaudiListenOptions(IPAddress.Any, 80);
        Assert.Equal(HttpProtocols.Http1AndHttp2, options.Protocols);
    }

    [Fact(Timeout = 5000)]
    public void GaudiListenOptions_should_not_be_https_by_default()
    {
        var options = new GaudiListenOptions(IPAddress.Loopback, 80);
    ...
    {
        var options = new GaudiListenOptions(IPAddress.Loopback, 443);
        options.UseHttps();
