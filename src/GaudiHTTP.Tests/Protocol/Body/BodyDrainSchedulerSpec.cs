...
public sealed class BodyDrainSchedulerSpec
{
    ...
    public void Register_should_emit_body_when_window_available()
    {
        ...
        var flow = MakeFlow();
        var scheduler = new BodyDrainScheduler(target, flow, new CancellationTokenSource(), new ConnectionObjectPool(), chunkSize: 1 * 1024, hardCap: 16);
    ...
    public void RoundRobin_should_interleave_two_streams()
    {
        var target = new FakeTarget();
        var flow = MakeFlow();
        var scheduler = new BodyDrainScheduler(target, flow, new CancellationTokenSource(), new ConnectionObjectPool(), chunkSize: 64, hardCap: 16);
    ...
    public void ForceAsync_should_dispatch_sync_reads_via_message()
    {
        ...
        var scheduler = new BodyDrainScheduler(target, flow, new CancellationTokenSource(), new ConnectionObjectPool(), chunkSize: 16, hardCap: 16);
    ...
    public void ConnectionWindowCeiling_should_cap_effectiveSlots()
    {
        ...
        var flow = MakeFlow(connWindow: 65535);
        var scheduler = new BodyDrainScheduler(target, flow, new CancellationTokenSource(), new ConnectionObjectPool(), chunkSize: 16 * 1024, hardCap: 16);
    ...
    public void CtsLifecycle_should_create_once_and_dispose_on_complete()
    {
        ...
        var scheduler = new BodyDrainScheduler(target, flow, connCts, new ConnectionObjectPool(), chunkSize: 1 * 1024, hardCap: 16);
