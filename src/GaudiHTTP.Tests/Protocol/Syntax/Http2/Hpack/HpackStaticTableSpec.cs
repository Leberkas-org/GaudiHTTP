...
public sealed class HpackStaticTableSpec
{
    [Fact(Timeout = 5000)]
    [Trait("RFC", "RFC7541-A")]
    ...
    public void HpackStaticTable_should_have_correct_name_and_value_at_index(int index, string expectedName,
    ...
    [Trait("RFC", "RFC7541-A")]
    public void
        HpackEncoder_should_encode_and_decode_correctly_when_round_tripping_all_pseudo_headers_via_static_table()
    {
        var encoder = new HpackEncoder(useHuffman: false);
        var decoder = new HpackDecoder();
    ...
    public void HpackDecoder_should_resolve_correctly_when_decoding_all_static_indices()
    {
        ...
        for (var idx = 1; idx <= HpackStaticTable.StaticCount; idx++)
        {
            ...
            Assert.True(expected.Name == result[0].Name,
                $"Name mismatch at index {idx}: expected '{expected.Name}', got '{result[0].Name}'");
            Assert.True(expected.Value == result[0].Value,
                $"Value mismatch at index {idx}: expected '{expected.Value}', got '{result[0].Value}'");
        }
    }
