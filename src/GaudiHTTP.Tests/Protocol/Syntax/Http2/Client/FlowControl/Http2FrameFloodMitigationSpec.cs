...
public sealed class Http2FrameFloodMitigationSpec
{
    ...
    public void Http2FrameDecoder_should_throw_http2_exception_when_101_settings_frames_received()
    {
        ...
        // Decode the 101st
        var framesAgain = decoder.Decode(settingsBytes.ToWireBuffer());
        ...
        Assert.Throws<HttpProtocolException>(() => EnforceSettingsFloodThreshold(settingsCount));
    }
    ...
    public void Http2FrameDecoder_should_not_count_settings_ack_toward_flood_threshold()
    {
        ...
        for (var i = 0; i < 200; i++)
        {
            var frames = decoder.Decode(settingsAck.ToWireBuffer());
            foreach (var frame in frames)
            {
                if (frame is SettingsFrame { IsAck: false })
                {
    ...
    public void Http2FrameDecoder_should_throw_http2_exception_when_101_rst_stream_received()
    {
        ...
        for (var i = 0; i < 100; i++)
        {
            ...
            var frames = decoder.Decode(rst.ToWireBuffer());
            foreach (var frame in frames)
            {
        ...
        var framesAgain = decoder.Decode(rst101.ToWireBuffer());
        foreach (var frame in framesAgain)
        {
            if (frame is RstStreamFrame)
            {
        ...
        Assert.Throws<HttpProtocolException>(() => EnforceRstFloodThreshold(rstCount));
    }
    ...
    public void Http2FrameDecoder_should_include_cve_reference_in_rapid_reset_message()
    {
        ...
        var ex = Assert.Throws<HttpProtocolException>(() => EnforceRstFloodThreshold(rstCount));
    ...
    public void Http2FrameDecoder_should_throw_http2_exception_when_1000_continuation_frames_received()
    {
        ...
        var frames = decoder.Decode(chunk.ToWireBuffer());
        ...
        var frames1000 = decoder.Decode(continuation1000.ToWireBuffer());
        ...
        Assert.Throws<HttpProtocolException>(() => EnforceContinuationFloodThreshold(continuationCount));
    }
    ...
    public void Http2FrameDecoder_should_throw_http2_exception_when_1001_ping_frames_received()
    {
        ...
        {
            var frames = decoder.Decode(pingFrame.ToWireBuffer());
            foreach (var frame in frames)
            {
        ...
        var frames1001 = decoder.Decode(pingFrame.ToWireBuffer());
        ...
        Assert.Throws<HttpProtocolException>(() => EnforcePingFloodThreshold(pingCount));
    }
    ...
    public void Http2FrameDecoder_should_not_count_ping_ack_toward_flood_threshold()
    {
        ...
        {
            var frames = decoder.Decode(pingAckFrame.ToWireBuffer());
            foreach (var frame in frames)
            {
                if (frame is PingFrame { IsAck: false })
                {
    ...
    public void Http2FrameDecoder_should_include_context_in_ping_flood_message()
    {
        ...
        var ex = Assert.Throws<HttpProtocolException>(() => EnforcePingFloodThreshold(pingCount));
