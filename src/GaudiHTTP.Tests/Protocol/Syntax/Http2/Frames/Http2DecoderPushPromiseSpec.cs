...
public sealed class Http2DecoderPushPromiseSpec
{
    ...
    public void Http2FrameDecoder_should_parse_push_promise_on_stream_zero_without_error()
    {
        ...
        var frames = new FrameDecoder().DecodeAll(new ReadOnlySequence<byte>(raw), out _);
        Assert.NotEmpty(frames);
    ...
    [Fact(Timeout = 5000)]
    [Trait("RFC", "RFC9113-6.6")]
    public void Http2FrameDecoder_should_decode_push_promise_with_even_promised_stream_id()
    {
        var hpack = new HpackEncoder(useHuffman: false);
        var headerBlock = hpack.Encode([(":path", "/resource")]);
        var frame = new PushPromiseFrame(1, 2, headerBlock).Serialize();
        ...
        Assert.Equal(0, pp.PromisedStreamId % 2);
    }

    [Fact(Timeout = 5000)]
    [Trait("RFC", "RFC9113-6.6")]
    public void Http2FrameDecoder_should_decode_consecutive_push_promises_with_increasing_stream_ids()
    {
        var hpack = new HpackEncoder(useHuffman: false);
        var decoder = new FrameDecoder();

        var block1 = hpack.Encode([(":path", "/a")]);
        var frames1 = decoder.DecodeAll(new ReadOnlySequence<byte>(new PushPromiseFrame(1, 2, block1).Serialize()), out _);
        var pp1 = Assert.IsType<PushPromiseFrame>(frames1[0]);

        var block2 = hpack.Encode([(":path", "/b")]);
        var frames2 = decoder.DecodeAll(new ReadOnlySequence<byte>(new PushPromiseFrame(1, 4, block2).Serialize()), out _);
        var pp2 = Assert.IsType<PushPromiseFrame>(frames2[0]);

        Assert.True(pp2.PromisedStreamId > pp1.PromisedStreamId);
    }

    [Fact(Timeout = 5000)]
    [Trait("RFC", "RFC9113-6.6")]
    public void Http2FrameDecoder_should_decode_push_promise_with_max_valid_stream_id()
    {
        var hpack = new HpackEncoder(useHuffman: false);
        var headerBlock = hpack.Encode([(":path", "/max")]);
        var frame = new PushPromiseFrame(1, 0x7FFFFFFE, headerBlock).Serialize();

        var frames = new FrameDecoder().DecodeAll(new ReadOnlySequence<byte>(frame), out _);

        var pp = Assert.IsType<PushPromiseFrame>(frames[0]);
        Assert.Equal(0x7FFFFFFE, pp.PromisedStreamId);
    }

    [Fact(Timeout = 5000)]
    [Trait("RFC", "RFC9113-6.6")]
    public void Http2FrameDecoder_should_decode_push_promise_without_end_headers_requiring_continuation()
    {
        var hpack = new HpackEncoder(useHuffman: false);
        var headerBlock = hpack.Encode([(":path", "/partial")]);
        var frame = new PushPromiseFrame(1, 2, headerBlock, endHeaders: false).Serialize();
