...
public sealed class Http2FrameDecoderStreamConstraintSpec
{
    [Fact(Timeout = 5000)]
    ...
    public void Http2FrameDecoder_should_throw_protocol_error_when_headers_without_end_headers_followed_by_ping()
    {
        ...
        var decoder = new FrameDecoder();
        Assert.Throws<HttpProtocolException>(() => decoder.DecodeAll(new ReadOnlySequence<byte>(combined), out _));
    }
}
