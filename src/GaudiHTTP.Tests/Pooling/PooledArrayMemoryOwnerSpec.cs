...
public sealed class PooledArrayMemoryOwnerSpec
{
    ...
    public void Memory_is_at_least_the_requested_length()
    {
        ...
        using var owner = PooledArrayMemoryOwner.Create(pool, 100);
    ...
    {
        var pool = ArrayPool<byte>.Create();
        var owner1 = PooledArrayMemoryOwner.Create(pool, 1024);
        var array1 = BackingArray(owner1.Memory);
        owner1.Dispose();

        using var owner2 = PooledArrayMemoryOwner.Create(pool, 1024);
    ...
    public async Task Buffer_returned_on_another_thread_is_reused()
    {
        ...
        var pool = ArrayPool<byte>.Create();
        var owner1 = PooledArrayMemoryOwner.Create(pool, 4096);
        var array1 = BackingArray(owner1.Memory);

        await Task.Run(() => owner1.Dispose(), TestContext.Current.CancellationToken);

        using var owner2 = PooledArrayMemoryOwner.Create(pool, 4096);
    ...
    {
        var pool = ArrayPool<byte>.Create();
        var owner = PooledArrayMemoryOwner.Create(pool, 512);
        owner.Dispose();
        owner.Dispose();
        ...
        using var a = PooledArrayMemoryOwner.Create(pool, 512);
        using var b = PooledArrayMemoryOwner.Create(pool, 512);

        Assert.NotSame(BackingArray(a.Memory), BackingArray(b.Memory));
    }

    [Fact(Timeout = 5000)]
    public void Shared_pool_create_rents_a_usable_owner()
    {
        using var owner = PooledArrayMemoryOwner.Create(2048);
