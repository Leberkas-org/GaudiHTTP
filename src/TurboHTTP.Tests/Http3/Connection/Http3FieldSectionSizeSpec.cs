...
public sealed class Http3FieldSectionSizeSpec
{
    ...
    public void ResponseDecoder_should_reject_oversized_field_section()
    {
        ...
        var ex = Assert.Throws<Http3Exception>(() => decoder.DecodeHeaders(headerFrame, state));
    ...
    {
        var tableSync = new QpackTableSync(encoderMaxCapacity: 0);
        tableSync.RemoteMaxFieldSectionSize = 32;

        var encoder = new RequestEncoder(tableSync);
        ...
        Assert.Throws<Http3Exception>(() => encoder.Encode(request));
    }
    ...
    {
        var tableSync = new QpackTableSync(encoderMaxCapacity: 0);
        tableSync.RemoteMaxFieldSectionSize = 65536;
