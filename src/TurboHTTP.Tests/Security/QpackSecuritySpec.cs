...
public sealed class QpackSecuritySpec
{
    ...
    public void HpackDecoder_should_reject_large_payloads_when_decoder_has_tight_limits()
    {
        ...
        var encoded = buf.AsSpan(0, written).ToArray();
        var ex = Assert.Throws<HpackException>(() => decoder.Decode(encoded));
    ...
    public void HpackEncoder_should_never_index_sensitive_headers_when_table_under_pressure()
    {
        ...
        var written1 = encoder.Encode(headers, ref bufSpan, useHuffman: false);

        var decoded = decoder.Decode(buf.AsSpan(0, written1));
        ...
        Span<byte> buf2Span = buf2;
        var written2 = encoder.Encode(headers, ref buf2Span, useHuffman: false);
        ...
        Assert.True(written2 >= written1 - 10,
