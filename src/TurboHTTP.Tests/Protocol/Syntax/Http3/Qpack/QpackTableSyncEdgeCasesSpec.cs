...
public sealed class QpackTableSyncEdgeCasesSpec
{
    ...
    public void Should_Apply_Multiple_EncoderInstructions()
    {
        ...
        var count1 = sync.ProcessEncoderInstructions(instructions1.Span);
        var count2 = sync.ProcessEncoderInstructions(instructions2.Span);
    ...
    public void Should_Update_EncoderKnownReceivedCount_OnSectionAck()
    {
        ...
        var n = QpackDecoderInstructionWriter.WriteSectionAcknowledgment(streamId: 5, ref writer);
    ...
    public void Should_Process_InsertCountIncrement_InDecoderInstructions()
    {
        ...
        var buffer = new byte[16];
        var writer = SpanWriter.Create(buffer);
        var n = QpackDecoderInstructionWriter.WriteInsertCountIncrement(5, ref writer);
    ...
    public void Should_Remove_Only_Cancelled_BlockedStream()
    {
        ...
        var writer = SpanWriter.Create(cancelBuf);
        var n = QpackDecoderInstructionWriter.WriteStreamCancellation(1, ref writer);
    ...
    public void Should_Resolve_Only_Ready_BlockedStreams()
    {
        ...
        sync.ProcessEncoderInstructions(enc1);
    ...
    public void Should_Resolve_All_BlockedStreams_When_ConditionMet()
    {
        ...
        sync.ProcessEncoderInstructions(allInstructions.ToArray().AsSpan());
    ...
    public void Should_Return_Zero_Increment_When_NoChange()
    {
        ...
        var increment = sync.WriteInsertCountIncrement(ref writer);
    ...
    public void Should_Apply_SetDynamicTableCapacity_Instruction()
    {
        ...
        var buf = new byte[16];
        var writer = SpanWriter.Create(buf);
        var written = QpackEncoderInstructionWriter.WriteSetDynamicTableCapacity(1024, ref writer);

        // Decode and apply the instruction
        var instructionDecoder = new QpackInstructionDecoder();
        var instructions = instructionDecoder.DecodeAllEncoderInstructions(buf.AsSpan(0, written));
    ...
    public void Should_Apply_Duplicate_Instruction()
    {
        ...
        var buffer = new byte[16];
        var writer = SpanWriter.Create(buffer);
        var written = QpackEncoderInstructionWriter.WriteDuplicate(0, ref writer);
    ...
    public void Should_Apply_InsertWithNameReference_Static()
    {
        ...
        var written = QpackEncoderInstructionWriter.WriteInsertWithNameReference(
            nameIndex: 0, isStatic: true, value: "POST", ref writer);
        ...
        foreach (var instr in instructions)
        {
            if (instr.Type == EncoderInstructionType.InsertWithNameReference)
            {
                var name = instr.IsStatic
                    ...
                    : "dynamic";
                sync.Decoder.DynamicTable.Insert(name, instr.Value);
            }
        }
