...
public sealed class QpackDecoderSpec
{
    ...
    {
        var tmp = new byte[16];
        var writer = SpanWriter.Create(tmp);
        var n = QpackIntegerCodec.Encode(value, prefixBits, prefixFlags, ref writer);
        ...
        ArrayBufferWriter<byte> buf)
    {
        ...
        var writer = SpanWriter.Create(tmp);
        var n = QpackStringCodec.Encode(value, prefixBits, prefixFlags, useHuffman, ref writer);
        buf.Write(tmp.AsSpan(0, n));
    }
}