...
public sealed class Http2ServerEncoderOptionsSpec
{
    [Fact(Timeout = 5000)]
    [Trait("RFC", "RFC9113")]
    public void Default_should_hold_SharedHttpOptions_Default()
    {
        Assert.Same(SharedHttpOptions.Default, Http2ServerEncoderOptions.Default.Shared);
    }
    ...
    public void Validate_should_delegate_to_Shared()
    {
        ...
        var opts = Http2ServerEncoderOptions.Default with { Shared = bad };
    ...
    {
        var opts = Http2ServerEncoderOptions.Default with { MaxFrameSize = 100 };
        Assert.Throws<ArgumentException>(opts.Validate);
    }
}
