...
namespace TurboHTTP.Tests.Protocol.Syntax.Http2.Components;
...
public sealed class Http2ServerResponseBufferSpec
{
    ...
    private static byte[] BuildHeadersFrame(int streamId, ReadOnlyMemory<byte> headerBlock, bool endStream = false, bool endHeaders = true)
    {
    ...
    public void ServerResponseEncoder_ApplyClientSettings_should_update_max_frame_size()
    {
        ...
        encoder.ApplyClientSettings(new[] { (SettingsParameter.MaxFrameSize, 32768u) });
    ...
    public void ServerResponseEncoder_ApplyClientSettings_should_ignore_initial_window_size()
    {
        ...
        encoder.ApplyClientSettings(new[] { (SettingsParameter.InitialWindowSize, 32768u) });
