...
public sealed class Http10ClientEncoderSpec : TestKit
{
    private static Http10ClientEncoder MakeEncoder() =>
        new(Http10ClientEncoderOptions.Default, Http10Profile.Default);
    ...
    [Fact(Timeout = 5000)]
    [Trait("RFC", "RFC1945-5")]
    public void Encoder_should_strip_Host_header_on_HTTP10()
    {
        ...
        var written = MakeEncoder().Encode(buf, request, ActorRefs.Nobody);
        Assert.DoesNotContain("Host:", Encoding.ASCII.GetString(buf, 0, written));
    }
