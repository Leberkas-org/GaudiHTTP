...
public sealed class HeaderValidationSpec
{
    ...
    {
        Assert.Equal(expected, HeaderValidation.IsToken(Encoding.UTF8.GetBytes(value)));
    }
    ...
    {
        Assert.Equal(expected, HeaderValidation.IsValidFieldValue(Encoding.UTF8.GetBytes(value)));
    }
