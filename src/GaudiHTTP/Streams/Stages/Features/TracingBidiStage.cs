...
/// <see cref="GaudiClientInstrumentationExtensions.StartRequest"/> and stores it in
...
internal sealed class TracingBidiProcessor(IFeatureStageOperations ops)
{
    ...
    public void OnRequestPush(HttpRequestMessage request)
    {
        ...
        if (Tracing.Source.HasListeners()
            || Metrics.RequestCount().Enabled
            || Metrics.RequestDuration().Enabled)
        {
    ...
    public void OnRequestUpstreamFailure(Exception ex)
    {
        ...
        {
            Tracing.SetHttpError(_currentActivity, ex);
    ...
    public void OnResponsePush(HttpResponseMessage response)
    {
        ...
        if (request?.Options
                .TryGetValue(GaudiClientInstrumentationExtensions.RequestActivityKey, out var activity) == true)
        {
    ...
    {
        if (!Metrics.ActiveRequests().Enabled)
        {
            return;
        }

        var method = GaudiClientInstrumentationExtensions.NormalizeMethod(request.Method.Method);
    ...
    {
        if (!Metrics.ActiveRequests().Enabled || request is null)
        {
            return;
        }
    ...
    {
        if (!Metrics.RequestCount().Enabled && !Metrics.RequestDuration().Enabled)
        {
            return;
        }
        ...
        var port = request.RequestUri?.Port ?? 0;
        var scheme = request.RequestUri?.Scheme ?? "https";
        var protocolVersion = GaudiClientInstrumentationExtensions.FormatProtocolVersion(response.Version);
