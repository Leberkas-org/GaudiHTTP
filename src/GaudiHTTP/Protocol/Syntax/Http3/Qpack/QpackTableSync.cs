...
///      table via <see cref="ProcessEncoderInstructions"/>.
///
...
internal sealed class QpackTableSync
{
    ...
    public int ProcessEncoderInstructions(ReadOnlySpan<byte> data)
    {
    ...
    ///
    /// Call this after <see cref="ProcessEncoderInstructions"/> to unblock streams.
    /// </summary>
    /// <returns>
    ...
    /// </summary>
    /// <param name="writer">Destination span (sliced on return to exclude written bytes).</param>
    /// <returns>The increment value written, or 0 if no increment was needed.</returns>
    public int WriteInsertCountIncrement(ref SpanWriter writer)
    {
        ...
        if (increment > 0)
        {
            QpackDecoderInstructionWriter.WriteInsertCountIncrement(increment, ref writer);
