...
internal static class HuffmanCodec
{
    ...
    /// <summary>
    /// Returns the worst-case number of bytes needed to Huffman-encode <paramref name="inputLength"/> bytes.
    /// The longest HPACK code is 30 bits, so the worst case is ceil(inputLength * 30 / 8).
    /// </summary>
    public static int GetMaxEncodedLength(int inputLength) => (inputLength * 30 + 7) / 8;

    /// <summary>
    /// Returns the worst-case number of bytes produced by decoding a Huffman-encoded span of
    /// <paramref name="inputLength"/> bytes. The shortest HPACK code is 5 bits, so the
    /// worst case is ceil(inputLength * 8 / 5).
    /// </summary>
    public static int GetMaxDecodedLength(int inputLength) => (inputLength * 8 + 4) / 5;

    /// <summary>
    /// Computes the exact number of bytes needed to Huffman-encode <paramref name="input"/>
    /// without performing the actual encoding or allocating any memory.
    /// Uses the HPACK Huffman symbol table to sum bit lengths.
    /// </summary>
    ...
    /// <summary>
    /// Huffman-encodes <paramref name="input"/> into <paramref name="output"/>.
    /// Returns the number of bytes written. The caller must ensure <paramref name="output"/>
    /// is at least <see cref="GetMaxEncodedLength"/> bytes long.
    /// </summary>
