...
internal sealed class BodyBridgeStream(BridgedBodyReader bridge) : Stream
{
    ...
    public override async ValueTask<int> ReadAsync(Memory<byte> buffer, CancellationToken cancellationToken = default)
    {
        ...
        if (_current.IsEmpty)
        {
            var result = await bridge.ReadAsync(cancellationToken).ConfigureAwait(false);
            if (result is { IsCompleted: true, Memory.IsEmpty: true })
            {
                _done = true;
                return 0;
    ...
    {
        var vt = bridge.ReadAsync(CancellationToken.None);
        if (!vt.IsCompleted)
        {
            throw new InvalidOperationException(
                "BridgedBodyReader.ReadAsync not completed synchronously — use ReadAsync on the stream.");
        }
