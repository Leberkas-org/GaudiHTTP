...
internal sealed class HpackEncoder
{
    ...
    private int EncodeHeader(HpackHeader header, ref Span<byte> output, bool useHuffman)
    {
        ...
        var encoding = header.NeverIndex || WellKnownHeaders.IsSensitiveHeaderName(header.Name)
        ...
        bool useHuffman)
    {
        ...
        written += encoding switch
        {
            HpackEncoding.IncrementalIndexing =>
                // RFC 7541 §6.2.1 – bit pattern: 01xxxxxx, prefix 6 bits
                WriteInteger(nameIndex, prefixBits: 6, prefixFlags: 0x40, ref output),
            HpackEncoding.WithoutIndexing =>
                // RFC 7541 §6.2.2 – bit pattern: 0000xxxx, prefix 4 bits
                WriteInteger(nameIndex, prefixBits: 4, prefixFlags: 0x00, ref output),
            HpackEncoding.NeverIndexed =>
                // RFC 7541 §6.2.3 – bit pattern: 0001xxxx, prefix 4 bits
                WriteInteger(nameIndex, prefixBits: 4, prefixFlags: 0x10, ref output),
