...
internal sealed class QpackDecoder
{
    ...
    public IReadOnlyList<(string Name, string Value)> Decode(ReadOnlySpan<byte> data, int streamId = 0)
    {
        ...
        _instructionOwner = MemoryPool<byte>.Shared.Rent(64);
        ...
        {
            var instrSpan = _instructionOwner!.Memory.Span[_instructionBytesWritten..];
            _instructionBytesWritten += QpackDecoderInstructionWriter.WriteSectionAcknowledgment(streamId, ref instrSpan);
        }
    ...
    /// </summary>
    private (string Name, string Value) DecodeLiteralWithNameReference(ReadOnlySpan<byte> data, ref int pos, int encodingBase)
    {
        ...
        var valueBytes = QpackStringCodec.Decode(data, ref pos, 7);
    ...
    /// </summary>
    private (string Name, string Value) DecodeLiteralWithPostBaseNameReference(ReadOnlySpan<byte> data, ref int pos, int encodingBase)
    {
    ...
    private static (string Name, string Value) DecodeLiteralWithoutNameReference(ReadOnlySpan<byte> data, ref int pos)
    {
        ...
        var nameBytes = QpackStringCodec.Decode(data, ref pos, 3);
    ...
    private static (string Name, string Value) LookupStaticEntry(int index)
    {
        if (index < 0 || index >= QpackStaticTable.Count)
        {
            throw new QpackException($"RFC 9204 §3.1 violation: Static table index {index} out of range (0–{QpackStaticTable.Count - 1}).");
        }
