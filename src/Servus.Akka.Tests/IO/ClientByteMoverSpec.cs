...
{
    [Fact(Timeout = 5000)]
    public async Task ClientByteMover_should_complete_inbound_pipe_on_stream_read()
    {
    ...
    [Fact(Timeout = 5000)]
    public async Task ClientByteMover_should_write_data_to_inbound_pipe()
    {
        ...
        Assert.True(state.InboundReader.TryRead(out var io));
        Assert.Equal(2, io.Length);
        Assert.Equal(0xAB, io.Span[0]);
        Assert.Equal(0xCD, io.Span[1]);
        io.Dispose();
    }
    ...
    [Fact(Timeout = 5000)]
    public async Task ClientByteMover_should_handle_fill_inbound_pipe_cancellation()
    {
        var stream = new MemoryStream([0x42], writable: false);
        var state = new ClientState(stream);
    ...
    [Fact(Timeout = 5000)]
    public async Task ClientByteMover_should_complete_pipe_with_exception_on_read_error()
    {
    ...
    [Fact(Timeout = 5000)]
    public async Task ClientByteMover_should_handle_drain_outbound_pipe_write_exception()
    {
        var stream = new FailingStream();
        var state = new ClientState(stream);
