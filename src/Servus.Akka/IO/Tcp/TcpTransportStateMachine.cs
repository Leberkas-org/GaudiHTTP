...
public sealed class TcpTransportStateMachine
{
    ...
    private void HandleConnectItem(ConnectItem connect)
    {
        ...
        {
            _ops.Log.Debug("TcpConnectionStage: ConnectItem (reconnect) key={0}:{1}", connect.Key.Host,
                connect.Key.Port);
    ...
    private void OnLeaseAcquired(ConnectionLease lease)
    {
        ...
        ServusMetrics.RequestTimeInQueue.Record(waitDurationS,
    ...
    private void OnAcquisitionFailed(Exception ex)
    {
        ...
        {
            ServusInstrumentation.SetError(_waitActivity, ex);
            _waitActivity.Stop();
            _waitActivity = null;
        }
    ...
    private void AcquireConnection(ConnectItem connect)
    {
        ...
        _waitActivity = ServusInstrumentation.StartWaitForConnection(
