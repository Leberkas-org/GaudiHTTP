...
public sealed class CacheH10IntegrationTests
{
    private readonly KestrelFixture _fixture;

    public CacheH10IntegrationTests(KestrelFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    /// client per request but share the same <see cref="HttpCacheStore"/> to verify
    /// caching persistence across HTTP/1.0 connections.
    /// </summary>
    private ClientHelper CreateCacheClient(HttpCacheStore store, CachePolicy? policy = null)
    {
    ...
    public async Task MaxAge_Response_Served_From_Cache()
    {
        ...
        var store = new HttpCacheStore(CachePolicy.Default);
