...
description: "Section 3.5: Solicited State Transitions — RFC 9000 — QUIC: A UDP-Based Multiplexed and Secure Transport"
...
> **SHOULD**: SHOULD signal this by sending a STOP_SENDING frame to prompt closure
...
> **MUST**: MUST send a RESET_STREAM frame if the stream is in the "Ready" or
   "Send" state.  If the stream is in the "Data Sent" state, the
> **MAY**: endpoint MAY defer sending the RESET_STREAM frame until the packets
   containing outstanding data are acknowledged or declared lost.  If
> **SHOULD**: any outstanding data is declared lost, the endpoint SHOULD send a
   RESET_STREAM frame instead of retransmitting the data.

> **SHOULD**: An endpoint SHOULD copy the error code from the STOP_SENDING frame to
   the RESET_STREAM frame it sends, but it can use any application error
> **MAY**: code.  An endpoint that sends a STOP_SENDING frame MAY ignore the
   error code in any RESET_STREAM frames subsequently received for that
   stream.

> **SHOULD**: STOP_SENDING SHOULD only be sent for a stream that has not been reset
