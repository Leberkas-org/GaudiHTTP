---
title: "15.3.  Successful 2xx"
rfc_number: 9110
rfc_section: "15.3"
source_url: "https://www.rfc-editor.org/rfc/rfc9110"
description: "Section 15.3: Successful 2xx — RFC 9110 — HTTP Semantics"
tags: [RFC9110, HTTP-semantics, methods, status-codes, redirects, retries, content-negotiation, conditional-requests, successful_2xx]
---
...
> **SHOULD**: In 200 responses to GET or HEAD, an origin server SHOULD send any
...
> **MUST NOT**: provided, a server MUST NOT generate content in a 205 response.
...
> **MUST**: A client MUST inspect a 206 response's Content-Type and Content-Range
   field(s) to determine what parts are enclosed and whether additional
   requests are needed.

> **MUST**: A server that generates a 206 response MUST generate the following
...
> **SHOULD NOT**: header field SHOULD NOT generate other representation header fields
   beyond those required because the client already has a prior response
> **MUST**: containing those header fields.  Otherwise, a sender MUST generate
...
> **MUST NOT**: MUST NOT generate a Content-Range header field in the HTTP header
...
> **MUST**: the server MUST generate a Content-Range header field corresponding
...
> **SHOULD**: (OK) response, the server SHOULD generate that same Content-Type
...
> **MAY**: When multiple ranges are requested, a server MAY coalesce any of the
...
> **MUST NOT**: A server MUST NOT generate a multipart response to a request for a
   single range, since a client that does not request multiple parts
> **MAY**: might not support multipart responses.  However, a server MAY
...
> **MUST NOT**: client that cannot process a "multipart/byteranges" response MUST NOT
   generate a request that asks for multiple ranges.

> **SHOULD**: A server that generates a multipart response SHOULD send the parts in
...
> **MUST**: that receives a multipart response MUST inspect the Content-Range
...
> **MAY**: on a target resource MAY combine those responses into a larger
...
> **MUST**: response, except that the client MUST use other header fields
...
> **MUST**: representation, then the client MUST process the combined response as
...
> **MUST**: client MUST process the set of continuous ranges as one of the
