---
title: "4.6.  Server Push"
rfc_number: 9114
rfc_section: "4.6"
source_url: "https://www.rfc-editor.org/rfc/rfc9114"
description: "Section 4.6: Server Push — RFC 9114 — HTTP/3"
tags: [RFC9114, HTTP/3, QUIC, variable-length-frames, unidirectional-streams, QPACK, SETTINGS, GOAWAY, server-push, server_push]
---
...
> **MAY**: Not all requests can be pushed.  A server MAY push requests that have
...
> **MUST**: The server MUST include a value in the :authority pseudo-header field
...
> **MUST**: request, it MUST perform the same verification process it would do
   before sending a request for that origin on the connection; see
> **MUST NOT**: Section 3.3.  If this verification fails, the client MUST NOT
   consider the server authoritative for that origin.

> **SHOULD**: Clients SHOULD send a CANCEL_PUSH frame upon receipt of a
...
> **MUST NOT**: corresponding responses MUST NOT be used or cached.
...
> **MAY**: MAY be considered by user agents when deciding how to use pushed
   resources.

   Ordering of a PUSH_PROMISE frame in relation to certain parts of the
> **SHOULD**: response is important.  The server SHOULD send PUSH_PROMISE frames
...
> **SHOULD**: the pushed stream.  Clients SHOULD abort reading and discard data
