---
title: "9.  HTTP/2 Connections"
rfc_number: 9113
rfc_section: "9"
source_url: "https://www.rfc-editor.org/rfc/rfc9113"
description: "Section 9: HTTP/2 Connections — RFC 9113 — HTTP/2"
tags: [RFC9113, HTTP/2, binary-framing, streams, multiplexing, flow-control, SETTINGS, HPACK, GOAWAY, WINDOW_UPDATE, http2_connections]
---
...
> **SHOULD NOT**: Clients SHOULD NOT open more than one HTTP/2 connection to a given
...
> **MAY**: A client MAY open multiple connections to the same IP address and TCP
   port using different Server Name Indication [TLS-EXT] values or to
> **SHOULD**: provide different TLS client certificates but SHOULD avoid creating
...
> **SHOULD**: TCP connection, the terminating endpoint SHOULD first send a GOAWAY
...
> **MUST**: certificate presented by the server MUST satisfy any checks that the
...
> **SHOULD**: SHOULD be followed, with some additional restrictions that are
   specific to HTTP/2.

> **MUST**: The TLS implementation MUST support the Server Name Indication (SNI)
   [TLS-EXT] extension to TLS.  If the server is identified by a domain
> **MUST**: name [DNS-TERMS], clients MUST send the server_name TLS extension
...
> **MAY**: met.  An endpoint MAY immediately terminate an HTTP/2 connection that
   does not meet these TLS requirements with a connection error
   (Section 5.4.1) of type INADEQUATE_SECURITY.

> **MUST**: A deployment of HTTP/2 over TLS 1.2 MUST disable compression.  TLS
...
> **MUST**: Implementations MUST support ephemeral key exchange sizes of at least
...
> **SHOULD NOT**: A deployment of HTTP/2 over TLS 1.2 SHOULD NOT use any of the
   prohibited cipher suites listed in Appendix A.

> **MAY**: Endpoints MAY choose to generate a connection error (Section 5.4.1)
...
> **MUST NOT**: Implementations MUST NOT generate this error in reaction to the
...
> **MUST**: that use TLS 1.2 MUST support TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256
