...
description: "Section 6.8: GOAWAY — RFC 9113 — HTTP/2"
...
> **MUST NOT**: GOAWAY frame MUST NOT open additional streams on the connection,
...
> **SHOULD**: Endpoints SHOULD always send a GOAWAY frame before closing a
...
> **SHOULD**: connection SHOULD still send a GOAWAY frame before terminating the
   connection.
...
> **MUST**: An endpoint MUST treat a GOAWAY frame with a stream identifier other
...
> **MAY**: An endpoint MAY send multiple GOAWAY frames if circumstances change.
...
> **MUST NOT**: streams could have been acted upon.  Endpoints MUST NOT increase the
...
> **SHOULD**: connection SHOULD send an initial GOAWAY frame with the last stream
   identifier set to 2^31-1 and a NO_ERROR code.  This signals to the
   client that a shutdown is imminent and that initiating further
   requests is prohibited.  After allowing time for any in-flight stream
> **MAY**: creation (at least one round-trip time), the server MAY send another
...
> **MUST**: PUSH_PROMISE, and CONTINUATION frames MUST be minimally processed to
   ensure that the state maintained for field section compression is
> **MUST**: consistent (see Section 4.3); similarly, DATA frames MUST be counted
...
> **MAY**: Endpoints MAY append opaque data to the frame payload of any GOAWAY
...
> **MUST**: persistently stored debug data MUST have adequate safeguards to
